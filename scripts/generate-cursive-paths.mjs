// One-off dev script: converts fixed strings into cursive SVG path data using
// the Alex Brush font (SIL OFL), so the browser bundle never needs the font
// file or opentype.js itself. Re-run this manually if the strings change:
...
//     glyph/offset combinations, so every glyph is rendered at (0,0) and the
//     resulting commands are translated by hand instead.
...
const entries = [toEntry('solace', 'Solace'), toEntry('tagline', 'freedom at last')];
...
export const CURSIVE_PATHS: Record<'solace' | 'tagline', CursivePath> = {
${entries
  .map(
    (e) => `  ${e.key}: {
